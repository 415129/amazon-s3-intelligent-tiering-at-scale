...
def put_bucket_lifecycle_configuration_standard(Name, lifecycle_config):
    ...
    try:
        ...
        if lifecycle_config["Rules"][0]["ID"] == "MMSStdDelMarkerPolicy":
            for target in Rules:
                try:
                    ...
                    if (
                        target["Expiration"]["ExpiredObjectDeleteMarker"] == "Ture"
                        or target["ID"] not in stdpname
                    ):
                        print(
                            "0-Deleteing LCP from Bucket = "
                            + Name
                            + " ,LCP = "
                            + target["ID"]
                        )
            ...
            lifecycle_config["Rules"][0]["ID"] == "AbortIncompleteMultipartUploadsRule"
        ):
            for target in Rules:
                try:
                    # print(target['NoncurrentVersionExpiration']['NewerNoncurrentVersions'])
                    if (
                        target["AbortIncompleteMultipartUpload"]["DaysAfterInitiation"]
                        > 6
                        or target["ID"] not in stdpname
                    ):
                        print(
                            "1-Deleteing LCP from Bucket = "
                            + Name
                            + " ,LCP = "
                            + target["ID"]
                        )
            ...
            for target in Rules:
                try:
                    ...
                    if (
                        (
                            target["NoncurrentVersionExpiration"][
                                "NewerNoncurrentVersions"
                            ]
                            > 1
                            and target["NoncurrentVersionExpiration"]["NoncurrentDays"]
                            > 31
                        )
                        or target["Expiration"]["Days"] > 31
                        or target["ID"] not in stdpname
                    ):
            ...
            for target in Rules:
                try:
                    if (
                        target["NoncurrentVersionExpiration"]["NoncurrentDaystarget"]
                        == 1
                        and target["NoncurrentVersionExpiration"][
                            "NewerNoncurrentVersions"
                        ]
                        > 3
                    ) or target["ID"] not in stdpname:
        ...
        if err.response["Error"]["Code"] == "NoSuchLifecycleConfiguration":
            s3.put_bucket_lifecycle_configuration(
                Bucket=Name, LifecycleConfiguration={"Rules": lifecycle_config["Rules"]}
            )
        elif err.response["Error"]["Code"] == "InvalidRequest":
            print(Rules, err.response)
        elif err.response["Error"]["ArgumentName"] == "ID":
            print(
                err.response["Error"]["ArgumentValue"]
                + " already exists and skipping rule"
            )
...
def put_bucket_lifecycle_configuration_custom(Name, lifecycle_config):
    ...
    try:
            ...
            for target in Rules:
                try:
                    ...
                    if target["ID"] not in [
                        ...
                        if (
                            target["Transitions"]["StorageClass"] not in ["GLACIER_IR"]
                            and target["Filter"]["ObjectSizeGreaterThan"] > 131072
                        ):
                            print(
                                "C1-Deleteing LCP from Bucket = "
                                + Name
                                + " ,LCP = "
                                + target["ID"]
                            )
        ...
        if err.response["Error"]["Code"] == "NoSuchLifecycleConfiguration":
            ...
            print(
                err.response["Error"]["ArgumentValue"]
                + " already exsists and skipping rule"
            )
        elif err.response["Error"]["Code"] == "InvalidRequest":
            print(err.response)
            ...
            for Rule in Rules:
                for key, value in Rule.items():
                    if key == "Transitions":
                        ...
                        TransitionStatus.append(
                            "No changes made to S3 Lifecycle configuration"
                        )
        else:
            # Scenario #2
            policy = lifecycle_config
            for p in policy["Rules"]:
                for key, value in p.items():
                    if key == "Transitions":
                        ...
                        TransitionStatus.append(StorageClass)
                        TransitionStatus.append(
                            "Updated the existing Lifecycle with Transition rule to S3 INT"
                        )
            ...
            for p in policy["Rules"]:
                for key, value in p.items():
                    if key == "Transitions":
                        ...
                        TransitionStatus.append(StorageClass)
                        TransitionStatus.append(
                            "Added a new S3 Lifecycle Transition Rule to S3 INT"
                        )
            lcp = s3.put_bucket_lifecycle_configuration(
...
def updateBucketsLcpStd():
    ...
    for bucket in BucketName["Buckets"]:
        if (
            bucket["Name"] not in ignorelist
            and bucket["Name"] == "max-dbalab2-data-classification"
        ):
            Name = bucket["Name"]
            print(Name)
            ...
            put_bucket_lifecycle_configuration_standard(
                Name, AbortIncompleteMultipartUploadsRule
            )
        ...
        if COL[0].value == "ID":
            for i, cell in enumerate(COL):
                ...
                cell.comment = Comment(
                    "this is name of the lifecycle Rule", "Automation Team"
                )
        if COL[0].value == "AbortIncompleteMultipartUpload":
            for i, cell in enumerate(COL):
                if i == 0:
                    continue
                cell.comment = Comment(
                    """{'DaysAfterInitiation': 7}""", "Automation Team"
                )
        ...
        if COL[0].value == "Filter":
            for i, cell in enumerate(COL):
                if i == 0:
                    continue
                cell.comment = Comment(
                    """{'ObjectSizeGreaterThan': 131072}""", "Automation Team"
                )
        ...
        if COL[0].value == "NoncurrentVersionExpiration":
            for i, cell in enumerate(COL):
                if i == 0:
                    continue
                cell.comment = Comment(
                    """{'NoncurrentDays': 2555}""", "Automation Team"
                )
        if COL[0].value == "NoncurrentVersionTransitions":
            for i, cell in enumerate(COL):
                if i == 0:
                    continue
                cell.comment = Comment(
                    """[{'NoncurrentDays': 31, 'StorageClass': 'STANDARD_IA', 'NewerNoncurrentVersions': 31}]""",
                    "Automation Team",
                )
